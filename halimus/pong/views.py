...
def profile_view(request, user_id):
    ...
    total_goals = sum(match.score for match in match_history)
    avg_goals = total_goals / total_matches if total_matches > 0 else 0
    
    # Yüzde hesaplamaları
    win_percentage = round((total_wins / total_matches * 100) if total_matches > 0 else 0, 1)
    loss_percentage = round((total_losses / total_matches * 100) if total_matches > 0 else 0, 1)
